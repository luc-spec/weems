...
class AgenTUI:
    ...
    def ask_user(self, connection: OpenSnitchConnection) -> int:
        ...
        print(f"Destination: {connection.dst_host if connection.dst_host else connection.dst_ip}:{connection.dst_port}")
    ...
    def get_feedback(self, connection: OpenSnitchConnection, action: int) -> Optional[bool]:
        """
        ...
        if random() < 0.1:  # Only ask 10% of the time to avoid bothering the user
            print("\n" + "-" * 60)
            print(f"Feedback Request for {connection.process_path} → {connection.dst_host if connection.dst_host else connection.dst_ip}")
            print(f"Action taken: {Actions.to_str(action)}")
            response = input("Was this the right decision? (y/n/[skip]): ").strip().lower()
            if response == 'y':
                return True
            elif response == 'n':
                return False
