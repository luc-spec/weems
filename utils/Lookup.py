...
class TrafficFilter:
    def __init__(self, blocklist_dir, epsilon=0.1):
        ...
        self.belief = {
            "allowed": 0.51,
            "blocked": 0.49,
        }  # Initial belief about allowed/blocked states
    ...
    def load_blocklists(self, blocklist_dir):
        ...
        for filename in os.listdir(blocklist_dir):
            if filename.endswith(
                ".txt"
            ):  # Assuming the blocklist files are in .txt format
                category = filename.split(".")[
                    0
                ]  # Use the file name (without extension) as the category
                blocklist = set()
    ...
    def epsilon_greedy(self):
        ...
        if random.random() < self.epsilon:
            ...
            return (
                "block" if self.belief["blocked"] > self.belief["allowed"] else "allow"
            )
    ...
    def get_user_feedback(self, request, action_taken):
        ...
        print(
            f"Was the action '{action_taken}' for request to {request.destination_ip} correct? (good/bad): ",
            end="",
        )
...
requests = [
    ...
    Request(
        ...
        process_id="67890",
        destination_ip="affiliatecashpile.go2jump.org",
        destination_port="80",
        protocol="tcp",
        user_id="1000",
        process_args="wget http://affiliatecashpile.go2jump.org/badfile",
    ),
    Request(
        process_path="/usr/bin/python3",
        process_id="67890",
        destination_ip="088156060096.nidzica.vectranet.pl",
        destination_port="80",
        protocol="tcp",
        user_id="1000",
        process_args="wget http://088156060096.nidzica.vectranet.pl/badfile",
    ),
    Request(
        process_path="firefox",
        process_id="6789",
        destination_ip="0-0-0-0-0-0proxy.tserv.se",
        destination_port="443",
        protocol="TCP",
        user_id="user4",
        process_args="https://0-0-0-0-0-0proxy.tserv.se",
    ),
    Request(
        process_path="chrome",
        process_id="1234",
        destination_ip="104.27.135.50",
        destination_port="443",
        protocol="TCP",
        user_id="user1",
        process_args="http://armdl.adobe.com/",
    ),
    Request(
        process_path="chrome",
        process_id="2345",
        destination_ip="185.221.88.122",
        destination_port="443",
        protocol="TCP",
        user_id="user2",
        process_args="https://www.8cr.purredheanb.online",
    ),
    Request(
        process_path="firefox",
        process_id="5678",
        destination_ip="104.26.12.50",
        destination_port="443",
        protocol="TCP",
        user_id="user3",
        process_args="https://tracking-site.net",
    ),
    Request(
        process_path="firefox",
        process_id="67890",
        destination_ip="1-170-195-217.cust.centrio.cz",
        destination_port="80",
        protocol="tcp",
        user_id="1001",
        process_args="wget http://1-170-195-217.cust.centrio.cz/badfile",
    ),
    Request(
        process_path="firefox",
        process_id="6789",
        destination_ip="08.185.87.118.liveadvert.com",
        destination_port="443",
        protocol="TCP",
        user_id="user5",
        process_args="https://08.185.87.118.liveadvert.com",
    ),
    Request(
        process_path="firefox",
        process_id="12345",
        destination_ip="1-2fly-befragung.de",
        destination_port="80",
        protocol="TCP",
        user_id="user6",
        process_args="http://1-2fly-befragung.de",
    ),
    Request(
        process_path="/usr/bin/python3",
        process_id="67890",
        destination_ip="0-0-0-6te.net",
        destination_port="80",
        protocol="tcp",
        user_id="1000",
        process_args="wget http://0-0-0-6te.net/badfile",
    ),
    Request(
        process_path="/usr/bin/python3",
        process_id="67890",
    ...
    Request(
        ...
        protocol="TCP",
        user_id="user4",
        process_args="https://phishing-page.net",
    ),
    Request(
        process_path="chrome",
        process_id="1234",
        ...
        protocol="TCP",
        user_id="user1",
        process_args="https://www.google.com",
    ),
    Request(
        process_path="chrome",
        process_id="2345",
    ...
    Request(
        ...
        protocol="TCP",
        user_id="user3",
        process_args="https://www.cloudflare.com",
    ),
    Request(
        process_path="chrome",
        process_id="6789",
