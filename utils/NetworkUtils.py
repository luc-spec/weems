...
def get_current_network_load(interface=None, interval=1.0, max_bandwidth=None):
    ...
    if interface is None:
        ...
        if interface not in initial_counters:
            raise ValueError(
                f"Interface {interface} not found. Available interfaces: {list(initial_counters.keys())}"
            )
        initial_bytes = (
            initial_counters[interface].bytes_sent
    ...
    if interface is None:
        ...
        final_bytes = (
            final_counters[interface].bytes_sent + final_counters[interface].bytes_recv
        )
