...
class NetworkLogAnalyzer:
        ...
        try:
            with open(self.log_file_path, 'r') as file:
                for line in file:
                    ...
                    if conn_match:
                        timestamp, component, level, conn_num, total_conn, process, dest_host, dest_port, action = conn_match.groups()
                    ...
                    if rule_match:
                        ...
                        try:
                            rule_data = eval(rule_json)  # Note: using eval for simplicity, but in production should use safer method
            ...
            if self.connections:
                ...
                self.df['duration'] = self.df['duration'].apply(lambda x: ' '.join(x) if x else None)
                
                print(f"Loaded {len(self.connections)} connection records and {len(self.rules)} rules.")
                self.loaded = True
            else:
    ...
    def plot_process_host_heatmap(self, top_n=10, output_path=None):
        ...
        top_hosts = self.df['destination_host'].value_counts().head(top_n).index.tolist()
        
        # Filter data to only include top processes and hosts
        filtered_df = self.df[self.df['process'].isin(top_processes) & self.df['destination_host'].isin(top_hosts)]
    ...
    def plot_blocked_vs_allowed_by_process(self, top_n=10, output_path=None):
        ...
        grouped = filtered_df.groupby(['process', 'actionType']).size().unstack(fill_value=0)
    ...
    def analyze_potentially_malicious_connections(self):
        ...
        malicious_df = self.df[self.df['destination_host'].str.contains('|'.join(malicious_indicators), case=False)]
    ...
    def generate_report(self, output_dir):
        ...
        self.plot_connections_over_time(output_path=output_dir / "connections_over_time.png")
        self.plot_action_distribution(output_path=output_dir / "action_distribution.png")
        self.plot_process_host_heatmap(top_n=10, output_path=output_dir / "process_host_heatmap.png")
        self.plot_blocked_vs_allowed_by_process(top_n=10, output_path=output_dir / "blocked_vs_allowed.png")
        self.plot_destination_port_distribution(top_n=10, output_path=output_dir / "port_distribution.png")
...
def main():
    ...
    parser.add_argument('--output', '-o', help='Output directory for the report', default='.weems/log_analysis_report')
    parser.add_argument('--export-only', action='store_true', help='Only export data to CSV without generating visualizations')
