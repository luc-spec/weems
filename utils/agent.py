...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("opensnitch_rl.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("opensnitch_rl")
...
class FeatureExtractor:
    ...
    def extract(self, connection: OpenSnitchConnection) -> np.ndarray:
        ...
        # 1. Process features
        process_features = self._extract_process_features(connection.process_path, connection.pid)
        features.extend(process_features)
        
        # 2. Network features
        network_features = self._extract_network_features(
            connection.dst_ip, connection.dst_host, connection.dst_port, connection.protocol)
    ...
    def _extract_process_features(self, process_path: str, pid: int) -> List[float]:
        ...
        if process_path in self.app_cache:
            ...
            time_known = (time.time() - app_data.get("first_seen", time.time())) / (86400 * 30)  # Normalize to ~30 days
            ...
            if app_data.get("connection_count", 0) > 0:
                block_ratio = app_data.get("blocks", 0) / app_data.get("connection_count", 1)
            else:
                block_ratio = 0.0
    ...
    def _extract_network_features(self, ip: str, host: str, port: int, protocol: str) -> List[float]:
    ...
    def _extract_system_features(self) -> List[float]:
        ...
        try:
            load_avg = os.getloadavg()[0] / 4.0  # Normalize by number of cores (assumed 4)
...
class RewardFunction:
    ...
    def calculate_reward(self, state: np.ndarray, action: int, 
                          ...
                          user_feedback: Optional[bool] = None) -> float:
        """
...
class RuleGenerator:
    ...
    def generate_rule(self, connection: OpenSnitchConnection, action: int) -> Dict:
        ...
        is_allow = action in [Actions.ALLOW_ONCE, Actions.ALLOW_TEMP, Actions.ALLOW_PERM]
...
class ThompsonSampling:
    ...
    def select_action(self, q_values: torch.Tensor, state_hash: str, 
                      exploit_threshold: float = 0.95) -> int:
        """
    ...
    def update(self, state_hash: str, action: int, reward: float):
        ...
        """
        if state_hash not in self.action_counts:
            self.action_counts[state_hash] = {a: {"success": self.alpha, "failure": self.beta} 
...
class OpenSnitchPolicy:
    ...
    def handle_connection(self, connection: OpenSnitchConnection) -> int:
        ...
        if self.training_mode and np.random.random() < 0.1:  # Exploration: ask user 10% of the time
        ...
        if action in [Actions.ALLOW_ONCE, Actions.ALLOW_TEMP, Actions.ALLOW_PERM] and \
           current_state[domain_reputation_idx] < self.reward_function.reputation_threshold_malicious:
            logger.warning(f"Allowing connection to potentially malicious site: {connection.dst_host}")
        
        # Return the chosen action
        return action
    
    def save_state(self, path: str = "data/opensnitch_rl_state.pkl"):
        ...
        state = {
            ...
            "exploration": {
                "action_counts": self.agent.exploration.action_counts
            }
        }
    ...
    def load_state(self, path: str = "data/opensnitch_rl_state.pkl"):
        ...
        try:
            ...
            self.feature_extractor.domain_cache = state["feature_extractor"]["domain_cache"]
            self.feature_extractor.app_cache = state["feature_extractor"]["app_cache"]
            self.feature_extractor.reputation_data = state["feature_extractor"]["reputation_data"]
...
def simulate_connections(policy: OpenSnitchPolicy, n_connections: int = 100):
    ...
    for i in range(n_connections):
        ...
        # Log the action
        logger.info(f"Connection {i+1}/{n_connections}: {app} → {domain or ip}:{port} | Action: {Actions.to_str(action)}")
...
class DRQNAgent:
    ...
    def get_action(self, app_id: str, state: np.ndarray, eval_mode: bool = False) -> int:
        """
        ...
        if len(state_seq) < self.sequence_length:
            # Pad with zeros
            padding = np.zeros((self.sequence_length - len(state_seq), state.shape[0]), dtype=np.float32)
    ...
    def update_model(self):
        ...
        for exp in experiences:
            ...
            batch_next_states.append(exp.next_state if exp.next_state is not None else exp.state)
        ...
        batch_next_states = torch.tensor(np.array(batch_next_states), dtype=torch.float32).to(self.device)
        batch_dones = torch.tensor(batch_dones, dtype=torch.float32).unsqueeze(1).to(self.device)
        ...
        target_hidden = (target_hidden[0].to(self.device), target_hidden[1].to(self.device))
        ...
        with torch.no_grad():
            ...
            expected_q_values = batch_rewards + self.gamma * next_q_values * (1 - batch_dones)
        ...
        loss = (weights * torch.torch.nn.functional.smooth_l1_loss(current_q_values, expected_q_values, reduction='none')).mean()
    ...
    def save_model(self, path: str = "models/opensnitch_rl_model.pt"):
        """Save model to disk"""
        # Create directory if it doesn't exist
        os.makedirs(os.path.dirname(path), exist_ok=True)
    ...
    def load_model(self, path: str = "models/opensnitch_rl_model.pt"):
...
def main():
    ...
    parser.add_argument("--simulate", type=int, default=0, help="Run simulation with N connections")
    parser.add_argument("--no-training", action="store_true", help="Disable training mode")
    parser.add_argument("--no-auto-rules", action="store_true", help="Disable automatic rule application")
    ...
    try:
        if args.simulate > 0:
            ...
            # TODO: In a real implementation, this would hook into OpenSnitch's event system
            print("Ready to handle connections. Press Ctrl+C to exit.")
            while True:
                time.sleep(1)
