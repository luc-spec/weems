...
class DBusSurrogate:
    ...
    def start_simulation(self, interval_range: tuple = (5, 15)) -> None:
        ...
        self._thread = threading.Thread(
            target=self._simulation_loop, args=(interval_range,)
        )
...
class DBusObjectSurrogate:
    ...
    def get_dbus_method(
        self, method_name: str, dbus_interface: Optional[str] = None
    ) -> Callable:
        """Get a surrogate method that can be called"""

        def method_surrogate(*args, **kwargs):
            print(
                f"Called method {method_name} on {self.service_name} ({dbus_interface})"
            )
...
def setup_opensnitch_surrogate_listener(callback_fn: Callable) -> DBusSurrogate:
    ...
    try:
        ...
        opensnitch_object = bus.get_object(
            "io.github.evilsocket.opensnitch", "/io/github/evilsocket/opensnitch/rule"
        )

        # Create a surrogate interface
        opensnitch_interface = DBusInterfaceSurrogate(
            opensnitch_object, "io.github.evilsocket.opensnitch.Rule"
        )
...
def handle_connection(event):
    ...
    print(
        f"  Destination: {event.destination_ip}:{event.destination_port} ({event.protocol})"
    )
